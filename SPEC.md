...
- `master`: 開発ブランチ
- `release`: リリース集約ブランチ
- 通常の開発フローは `feature/* -> master`
- リリース時のみ `master -> release` の PR を作成
- `master -> release` PR のタイトルは `v0.X.Xのreleaseまとめ` を使う
- `release` へ merge されたら GitHub Actions が `VERSION` を patch bump し、commit、tag、VST3 build、GitHub Release 作成まで行う
