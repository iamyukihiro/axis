...
void AxisCenterAudioProcessor::processBlock(juce::AudioBuffer<float> &buffer, juce::MidiBuffer &) {
    ...
    for (auto sample = 0; sample < buffer.getNumSamples(); ++sample) {
        ...
        const auto trim =
            autoGainEnabled ? applyAutoTrim(stagedLeft, stagedRight, outLeft, outRight) : 1.0f;
...
float AxisCenterAudioProcessor::applyAutoTrim(float inputLeft, float inputRight, float outputLeft,
